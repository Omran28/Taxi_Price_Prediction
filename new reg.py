...
taxiData = pd.read_csv('taxi-tas-regression-test.csv')
...
X_train_poly = poly_features.fit_transform(X2)

rain_model = pickle.load(open('rain_model.pkl', 'rb'))
prediction = rain_model.predict(X_train_poly)
...
# Remove duplicated
Values = mergedValues.drop_duplicates(subset=["destination", "location", "month", "day", "hour"],keep='first')  # 7877 rows
...
pickle.dump(one_hot_data, open("regression_encoder.joblib", "wb"))

# New values of cab type
Values["uber"] = one_hot_data[:, var]
Values["lyft"] = one_hot_data[:, 1]

Values = Values.drop(["cab_type", "destination", "id", "product_id", "time_stamp_x", "time_stamp_y", "year_x", "year_y", "minute_x",
...
select = pickle.load(open('regression_selection.pkl', 'rb'))
...
#seperate data
all_X_train, all_X_test, all_y_train, all_y_test = train_test_split(all_X, Y, test_size=0.30, shuffle=True, random_state=10)
all_poly_features = PolynomialFeatures(degree=3)
all_X_train_poly = all_poly_features.fit_transform(all_X_train)
# model.fit(x_poly, y)
#linear regression model
lin_model = pickle.load(open('regression_linear_model.pkl', 'rb'))

#BayesianRidge model
BR_model = pickle.load(open('regression_bayesian_model.pkl', 'rb'))


#prediction
lin_model_prediction = lin_model.predict(all_poly_features.fit_transform(all_X_test))
